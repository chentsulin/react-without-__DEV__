...
describe('ReactChildren', () => {
  ...
  it('should be called for each child in an iterable without keys', () => {
    ...
    expectDev(console.error.calls.argsFor(0)[0]).toContain('Warning: Each child in an array or iterator should have a unique "key" prop.');
  ...
  it('should pass key to returned component', () => {
    ...
    var instance = <div>{simpleKid}</div>;
    var mappedChildren = React.Children.map(instance.props.children, function (kid) {
      return <div>{kid}</div>;
    });
  ...
  it('should be called for each child', () => {
    ...
    var mapped = [<div key="giraffe" />, // Key should be joined to obj key
    ...
    expect([mappedChildren[0].key, mappedChildren[1].key, mappedChildren[2].key, mappedChildren[3].key]).toEqual(['giraffe/.$keyZero', '.$keyTwo', '.3', '.$keyFour']);
  ...
  it('should be called for each child in nested structure', () => {
    ...
    expect(React.Children.count(mappedChildren)).toBe(4);
    // Keys default to indices.
    expect([mappedChildren[0].key, mappedChildren[1].key, mappedChildren[2].key, mappedChildren[3].key]).toEqual(['giraffe/.0:0:$keyZero', '.0:0:$keyTwo', '.0:1:$keyFour', '.0:$keyFive']);
  ...
  it('should retain key across two mappings', () => {
    ...
    expect(mappedForcedKeys).toEqual(['giraffe/.$keyZero', '.$keyOne']);

    var remappedChildrenForcedKeys = React.Children.map(mappedChildrenForcedKeys, mapFn);
    expect(remappedChildrenForcedKeys.map(c => c.key)).toEqual(['giraffe/.$giraffe/.$keyZero', '.$.$keyOne']);
  });
  ...
  it('should use the same key for a cloned element', () => {
    ...
    var mappedWithClone = React.Children.map(instance.props.children, element => React.cloneElement(element));
  ...
  it('should use the same key for a cloned element with key', () => {
    ...
    var mappedWithClone = React.Children.map(instance.props.children, element => React.cloneElement(element, { key: 'unique' }));
  ...
  it('should count the number of children in nested structure', () => {
    ...
    var instance = <div>
        {[[[zero, null, two], [null, four], five], null]}
      </div>;
    var numberOfChildren = React.Children.count(instance.props.children);
  ...
  it('should flatten children to an array', () => {
    ...
    var flattened = React.Children.toArray([[<div key="apple" />, <div key="banana" />, <div key="camel" />], [<div key="banana" />, <div key="camel" />, <div key="deli" />]]);
    ...
    var reversed = React.Children.toArray([[<div key="camel" />, <div key="banana" />, <div key="apple" />], [<div key="deli" />, <div key="camel" />, <div key="banana" />]]);
  ...
  it('should throw on object', () => {
    ...
    }).toThrowError('Objects are not valid as a React child (found: object with keys ' + '{a, b}). If you meant to render a collection of children, use an ' + 'array instead.');
  });
  ...
  it('should throw on regex', () => {
    ...
    }).toThrowError('Objects are not valid as a React child (found: /abc/). If you meant ' + 'to render a collection of children, use an array instead.');
  });
  ...
  if (ReactDOMFeatureFlags.useFiber) {
    describe('with fragments enabled', () => {
      ...
      it('warns for keys for arrays of elements in a fragment', () => {
        ...
        expectDev(console.error.calls.count()).toBe(1);
        expectDev(normalizeCodeLocInfo(console.error.calls.argsFor(0)[0])).toBe('Warning: ' + 'Each child in an array or iterator should have a unique "key" prop.' + ' See https://fb.me/react-warning-keys for more information.' + '\n    in ComponentReturningArray (at **)');
      });
