...
function insertBefore(parentInstance: Instance | Container, child: Instance | TextInstance, beforeChild: Instance | TextInstance): void {
  const index = parentInstance.children.indexOf(child);
  if (index !== -1) {
    parentInstance.children.splice(index, 1);
  }
...
var TestRenderer = ReactFiberReconciler({
  ...
  createInstance(type: string, props: Props, rootContainerInstance: Container): Instance {
    return {
      type,
      props,
      children: [],
  ...
  commitUpdate(instance: Instance, updatePayload: {}, type: string, oldProps: Props, newProps: Props): void {
    instance.type = type;
...
const validWrapperTypes = new Set([FunctionalComponent, ClassComponent, HostComponent]);
...
class ReactTestInstance {
  ...
  _currentFiber(): Fiber {
    ...
    const fiber = ReactFiberTreeReflection.findCurrentFiberUsingSlowPath(this._fiber);
...
function findAll(root: ReactTestInstance, predicate: Predicate, options: ?FindOptions): Array<ReactTestInstance> {
