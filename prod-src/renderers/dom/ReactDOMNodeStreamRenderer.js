...
function renderToStream(element) {
  ...
  if (disableNewFiberFeatures) {
    invariant(React.isValidElement(element), 'renderToStream(): Invalid component element.');
  }
...
function renderToStaticStream(element) {
  const disableNewFiberFeatures = ReactFeatureFlags.disableNewFiberFeatures;
  if (disableNewFiberFeatures) {
    invariant(React.isValidElement(element), 'renderToStaticStream(): Invalid component element.');
  }
