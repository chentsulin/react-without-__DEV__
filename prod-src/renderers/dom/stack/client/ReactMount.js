...
var ReactMount = {
  ...
  _renderSubtreeIntoContainer: function (parentComponent, nextElement, container, callback) {
    ...
    var component = ReactMount._renderNewRootComponent(nextWrappedElement, container, containerHasReactMarkup && !prevComponent && !containerHasNonRootReactChild, nextContext, callback)._renderedComponent.getPublicInstance();
    return component;
  },
  ...
  _mountImageIntoNode: function (markup, container, instance, shouldReuseMarkup, transaction) {
    invariant(isValidContainer(container), 'mountComponentIntoNode(...): Target container is not valid.');
    ...
    if (shouldReuseMarkup) {
      ...
      if (ReactMarkupChecksum.canReuseMarkup(markup, rootElement)) {
        ...
        invariant(container.nodeType !== DOCUMENT_NODE, "You're trying to render a component to the document using " + 'server rendering but the checksum was invalid. This usually ' + 'means you rendered a different component type or props on ' + 'the client from the one on the server, or your render() ' + 'methods are impure. React cannot handle this case due to ' + 'cross-browser quirks by rendering at the document root. You ' + 'should look for environment dependent code in your components ' + 'and ensure the props are the same client and server side:\n%s', difference);
      }
    }

    invariant(container.nodeType !== DOCUMENT_NODE, "You're trying to render a component to the document but " + "you didn't use server rendering. We can't do this " + 'without using server rendering due to cross-browser quirks. ' + 'See ReactDOMServer.renderToString() for server rendering.');
