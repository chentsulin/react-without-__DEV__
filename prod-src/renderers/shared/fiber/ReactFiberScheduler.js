...
var warning, ReactFiberInstrumentation, ReactDebugCurrentFiber, {
  recordEffect,
...
}, warnAboutUpdateOnUnmounted, warnAboutInvalidUpdates;
...
module.exports = function <T, P, I, TI, PI, C, CX, PL>(config: HostConfig<T, P, I, TI, PI, C, CX, PL>) {
  ...
  const hydrationContext: HydrationContext<C> = ReactFiberHydrationContext(config);
  const { popHostContainer, popHostContext, resetHostContainer } = hostContext;
  const { beginWork, beginFailedWork } = ReactFiberBeginWork(config, hostContext, hydrationContext, scheduleUpdate, getPriorityContext);
  ...
  function resetNextUnitOfWork() {
    ...
    if (highestPriorityRoot !== null) {
      ...
      nextUnitOfWork = createWorkInProgress(highestPriorityRoot.current, highestPriorityLevel);
      return;
    }
  ...
  function commitAllWork(finishedWork: Fiber) {
    ...
    if (nextPriorityLevel === SynchronousPriority || nextPriorityLevel === TaskPriority) {
  ...
  function resetWorkPriority(workInProgress: Fiber, renderPriority: PriorityLevel) {
    if (workInProgress.pendingWorkPriority !== NoWork && workInProgress.pendingWorkPriority > renderPriority) {
  ...
  function handleCommitPhaseErrors() {
    ...
    if (capturedErrors !== null && capturedErrors.size > 0 && nextPriorityLevel === TaskPriority) {
      while (nextUnitOfWork !== null) {
        ...
        if (nextUnitOfWork === null) {
          invariant(pendingCommit !== null, 'Should have a pending commit. This error is likely caused by ' + 'a bug in React. Please file an issue.');
  ...
  function workLoop(minPriorityLevel: PriorityLevel, deadline: Deadline | null) {
      ...
      if (nextPriorityLevel <= TaskPriority) {
        ...
        while (nextUnitOfWork !== null) {
          ...
          if (nextUnitOfWork === null) {
            ...
            if (nextPriorityLevel === NoWork || nextPriorityLevel > minPriorityLevel || nextPriorityLevel > TaskPriority) {
          ...
          if (deadline.timeRemaining() > timeHeuristicForUnitOfWork) {
            ...
            if (nextUnitOfWork === null) {
              ...
              if (deadline.timeRemaining() > timeHeuristicForUnitOfWork) {
                ...
                handleCommitPhaseErrors();
                // The priority level may have changed. Check again.
                if (nextPriorityLevel === NoWork || nextPriorityLevel > minPriorityLevel || nextPriorityLevel < HighPriority) {
                  // The priority level does not match.
                  break;
                }
              } else {
  ...
  function performWorkCatchBlock(failedWork: Fiber, boundary: Fiber, minPriorityLevel: PriorityLevel, deadline: Deadline | null) {
  ...
  function performWork(minPriorityLevel: PriorityLevel, deadline: Deadline | null) {
  ...
  function scheduleUpdateImpl(fiber: Fiber, priorityLevel: PriorityLevel) {

    if (nestedUpdateCount > 1000) {
      didFatal = true;
  ...
  function getPriorityContext(fiber: Fiber, forceAsync: boolean): PriorityLevel {
  ...
  return {
    ...
    performWithPriority: function (priorityLevel: PriorityLevel, fn: Function) {
      const previousPriorityContext = priorityContext;
    ...
    batchedUpdates: function <A, R>(fn: (a: A) => R, a: A): R {
    ...
    unbatchedUpdates: function <A>(fn: () => A): A {
    ...
    flushSync: function <A>(batch: () => A): A {
      const previousIsBatchingUpdates = isBatchingUpdates;
      const previousPriorityContext = priorityContext;
      isBatchingUpdates = true;
    ...
    deferredUpdates: function <A>(fn: () => A): A {
      const previousPriorityContext = priorityContext;
